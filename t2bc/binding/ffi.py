...
from t2bc.binding.common import _decode_string, _is_shutting_down
...
class _lib_wrapper(object):
    """Wrap libt2bc with a lock such that only one thread may access it at
    a time.
...
class OutputString(object):
    ...
    def close(self):
        if self._ptr is not None:
            if self._owned:
                lib.T2BC_DisposeString(self._ptr)
            self._ptr = None
...
class OutputBytes(object):
    ...
    def from_return(cls, ptr):
        ...
        string, we must use *c_void_p* to obtain the raw pointer.
        """
        return cls(init=ctypes.cast(ptr, ctypes.c_void_p))
