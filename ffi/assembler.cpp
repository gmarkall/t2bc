...
assemble(LLVMContextRef context, const char *ir, const char **bitcode,
  ...
  std::unique_ptr<llvm::Module> m =
      llvm::parseAssemblyString(ir, error, *llvm::unwrap(context));
...
T2BC_CreateString(const char *msg) { return strdup(msg); }

API_EXPORT(void)
T2BC_DisposeString(const char *msg) { free(const_cast<char *>(msg)); }

API_EXPORT(LLVMContextRef)
T2BC_GetGlobalContext() { return LLVMGetGlobalContext(); }

API_EXPORT(LLVMContextRef)
T2BC_ContextCreate() { return LLVMContextCreate(); }

API_EXPORT(void)
T2BC_ContextDispose(LLVMContextRef context) { LLVMContextDispose(context); }
